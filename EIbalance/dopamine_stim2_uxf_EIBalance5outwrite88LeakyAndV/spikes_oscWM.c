...
float contrastE_sel=1.2f, contrastE_nonsel=1.0f;
...
float Tprestim_SA=10000, Tcue_SA=0, Tdelay_SA=0, Ttest_SA=0;	 
...
float TCamp = 10.0,TCamp1=10.0,TCamp2=1.0;      /* Tempo[ms] di campionamento attivita' neurale*/
...
int *spikes, spikesI;
float *curr, currI;
float *I_Exc,*I_Inh,*I_Sum,*I_ExtBg,*I_Leaky;
float theta; 
...
Neuron *neur;
EffRoot *J;         
float **Events,**EventsExc,**EventsInh;       
float **EventsNMDA;

FILE *rates, *curr_out, *ratepops, *spikeout, *synvar_u, *synvar_x,*num_para;

FILE *fI_Exc,*fI_Inh,*fI_Sum,*fI_ExtBg,*fV_V,*fI_Leaky;
unsigned int Time;
...
void InitEventMatrix()
{
  ...
  EventsExc=(float **)malloc(tbound*sizeof(*EventsExc));
  EventsInh=(float **)malloc(tbound*sizeof(*EventsInh));
  for(t=0; t<tbound; t++)
    {
      Events[t]=(float *)malloc((Ne+Ni)*sizeof(**Events));
      EventsNMDA[t]=(float *)malloc((Ne+Ni)*sizeof(**EventsNMDA));
      EventsExc[t]=(float *)malloc((Ne+Ni)*sizeof(**EventsExc));
      EventsInh[t]=(float *)malloc((Ne+Ni)*sizeof(**EventsInh));
...
void InitV()
{
  ...
  I_Exc=(float*) malloc( Ntot * sizeof(float));
  I_Inh=(float*) malloc( Ntot * sizeof(float));
  I_Sum=(float*) malloc( Ntot * sizeof(float));
  I_ExtBg=(float*) malloc( Ntot * sizeof(float));
  I_Leaky=(float*) malloc( Ntot * sizeof(float));
...
void single_trial(int cue)
{
  ...
  unsigned int tcamp,tcamp1,tcamp2,tsyn, tcampV;
  ...
  tcamp1=(unsigned int)ROUND(TCamp1/dt);
  tcamp2=(unsigned int)ROUND(TCamp2/dt);
  (void)select_VR(cue);
  ...
  while(Time<life)
	...
	{
	  fprintf(ratepops,"%.2f ",(Time+1)*dt);fflush(ratepops); 
	  fprintf(curr_out,"%.2f ",(Time+1)*dt); fflush(curr_out);
	  ...
	  for (p=0; p<pops; p++)
		...
		if (sel_old[i][p]==1)
	      ...
	      fprintf(curr_out,"%.4f ",tot_currE/TCamp/Nn);//每个兴奋性细胞团的递归电流（不含噪声输入） 
	    }
	  
	  fprintf(ratepops,"%.2f\n",(float)spikesI/TCamp/Ni*1000);fflush(ratepops); 
	  fprintf(curr_out,"%.4f\n",currI/TCamp/Ni); fflush(curr_out);
	...
	if (Time%tcamp1==tcamp1-1)
	{
	 for (i=0; i<Ntot; i++)
      {
	  	fprintf(fI_Exc,"%.4f\t  ",I_Exc[i]/TCamp1);fflush(fI_Exc);
   		fprintf(fI_Inh,"%.4f\t  ",I_Inh[i]/TCamp1);	fflush(fI_Inh);
   		fprintf(fI_Sum,"%.4f\t  ",I_Sum[i]/TCamp1); fflush(fI_Sum);
    	fprintf(fI_ExtBg,"%.4f\t  ",I_ExtBg[i]/TCamp1);fflush(fI_ExtBg);
   	    fprintf(fI_Leaky,"%.4f\t  ",I_Leaky[i]/TCamp1);fflush(fI_Leaky);
      
        I_Exc[i]=0;I_Inh[i]=0;I_Sum[i]=0;I_ExtBg[i]=0;I_Leaky[i]=0;  
  	  }
	 fprintf(fI_Exc,"22\n");fflush(fI_Exc);//22 means end,just a flag
	 fprintf(fI_Inh,"22\n");fflush(fI_Inh);
   	 fprintf(fI_Sum,"22\n"); fflush(fI_Sum);
	 fprintf(fI_ExtBg,"22\n");fflush(fI_ExtBg);
	 fprintf(fI_Leaky,"22\n");fflush(fI_Leaky);
	 ...
	 //if (Time%tcamp2==tcamp2-1)
	//{
	 for (i=0; i<Ntot; i=i+40)
      {
	  	fprintf(fV_V,"%.4f\t  ",neur[i].V);fflush(fV_V);
   		 
  	  }
	 fprintf(fV_V,"33\n");fflush(fV_V);//33 means end,just a flag
      ...
      for (i=0; i<Ntot; i++)   
	{
	  ...
	  //if(flag_SA==0)
	    // {
           ...
           I_Exc[i]+=EventsExc[Time%tbound][i];
	       I_Inh[i]+=EventsInh[Time%tbound][i];
		   I_Sum[i]+=Events[Time%tbound][i];
		   I_ExtBg[i]+=Iext[i];
	       I_Leaky[i]+=neur[i].V*(exp(-dt/tau)-1);
        // }
	  ...
	  if(neur[i].status>=0) /* ci troviamo in fase refrattaria? */
  	 ...
  	 EventsInh[Time%tbound][i]=0;
     EventsExc[Time%tbound][i]=0; 
  	 Events[Time%tbound][i]=0;
		...
		{  
		  ...
		  //if ((i%10==0))
		   // {
		      fprintf(spikeout,"%d\t%.2f\t",i,(Time*dt));fflush(spikeout);
		      if (i<Ne)
			{
			  fprintf(spikeout,"%d\t",sel_old[i][pops]);fflush(spikeout);
			  if (gg[i]==1)
			    fprintf(spikeout,"1\n");
			  else
			    fprintf(spikeout,"0\n");
			  ...
			  if(i2<Ne)
			    {
				  ...
				  EventsExc[(Time+deltat)%tbound][i2]+=((1-xNMDA_EE)*neur[i].uxtm*J[i].eff[n].efficacy);				
			    }
			  else
			    {
			      ...
			      EventsExc[(Time+deltat)%tbound][i2]+=(1-xNMDA_IE)*J[i].eff[n].efficacy;
			    } 
			}
		      else
			{Events[(Time+deltat)%tbound][i2]+=J[i].eff[n].efficacy;
			EventsInh[(Time+deltat)%tbound][i2]+=J[i].eff[n].efficacy;
			}
		    }
		  //update tspike 
		  neur[i].tspike=Time;
  ...
  for (nt=0; nt<1; nt++)
    {
     ...
     {sprintf(name,"rates_pops_%1d_%3d.log",nt,D1);
      ...
      sprintf(name,"I_Exc_%1d_%3d.log",nt,D1);
      fI_Exc=fopen(name,"w");
      sprintf(name,"I_Inh_%1d_%3d.log",nt,D1);
      fI_Inh=fopen(name,"w");
      sprintf(name,"I_Sum_%1d_%3d.log",nt,D1);
      fI_Sum=fopen(name,"w");
      sprintf(name,"I_ExtBg_%1d_%3d.log",nt,D1);
      fI_ExtBg=fopen(name,"w");
	  sprintf(name,"I_Leaky_%1d_%3d.log",nt,D1);
      fI_Leaky=fopen(name,"w");
      sprintf(name,"Vot_%1d_%3d.log",nt,D1);
      fV_V=fopen(name,"w");
     } 
     else 
     {
      ...
      curr_out=fopen(name,"w");
      sprintf(name,"I_Exc_%1d_%1d.log",nt,D1);
      fI_Exc=fopen(name,"w");
      sprintf(name,"I_Inh_%1d_%1d.log",nt,D1);
      fI_Inh=fopen(name,"w");
      sprintf(name,"I_Sum_%1d_%1d.log",nt,D1);
      fI_Sum=fopen(name,"w");
      sprintf(name,"I_ExtBg_%1d_%1d.log",nt,D1);
      fI_ExtBg=fopen(name,"w");
      sprintf(name,"I_Leaky_%1d_%1d.log",nt,D1);
      fI_Leaky=fopen(name,"w");
      sprintf(name,"Vot_%1d_%1d.log",nt,D1);
      fV_V=fopen(name,"w");
	  }
      else
      ...
      curr_out=fopen(name,"w");
      sprintf(name,"I_Exc_%1d_%2d.log",nt,D1);
      fI_Exc=fopen(name,"w");
      sprintf(name,"I_Inh_%1d_%2d.log",nt,D1);
      fI_Inh=fopen(name,"w");
      sprintf(name,"I_Sum_%1d_%2d.log",nt,D1);
      fI_Sum=fopen(name,"w");
      sprintf(name,"I_ExtBg_%1d_%2d.log",nt,D1);
      fI_ExtBg=fopen(name,"w");
      sprintf(name,"I_Leaky_%1d_%2d.log",nt,D1);
      fI_Leaky=fopen(name,"w");
      sprintf(name,"Vot_%1d_%2d.log",nt,D1);
      fV_V=fopen(name,"w");
      ...
      fflush(ratepops); fflush(curr_out);//释放内存磁盘内存文件， 
      fflush(spikeout);fflush(synvar_u);fflush(synvar_x);
      fflush(fI_Exc);fflush(fI_Inh);fflush(fI_Sum);fflush(fI_ExtBg);fflush(fI_Leaky);
      fflush(fV_V);

      fclose(ratepops); fclose(curr_out); 
      fclose(spikeout); fclose(synvar_u);  fclose(synvar_x);
      fclose(fI_Exc);fclose(fI_Inh);fclose(fI_Sum);fclose(fI_ExtBg);fclose(fI_Leaky);
      fclose(fV_V);
    }
