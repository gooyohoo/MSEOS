...
data0=load(['./dopamine_stim2_uxf1/mingw5/','num_parameter_0_',num2str(D1_start),'.log']);
...
for num1=Trial_start:Trial_end
    ...
    path=['./dopamine_stim2_uxf',num2str(num1),'/mingw5/'];
...
figure();
subplot(1,5,1);
errorbar(xx,MFe1,SFe1,'-');
xlabel('DA','FontWeight','demi','FontSize',12); 
ylabel('f[Hz]','FontWeight','demi','FontSize',12);
hold on;line([D1_refer,D1_refer],[0,MFe1(OptiDA)],'line','--');hold off;
ylim([0,16]);xlim([-0.2,2.2]);
subplot(1,5,2);
errorbar(xx,Mu_stim_end,Su_stim_end,'-');
xlabel('DA','FontWeight','demi','FontSize',12); 
ylabel('u','FontWeight','demi','FontSize',12);
hold on;line([D1_refer,D1_refer],[0,Mu_stim_end(OptiDA)],'line','--');hold off;
ylim([0,1]);xlim([-0.2,2.2]);
subplot(1,5,3);
errorbar(xx,Mx_stim_end,Sx_stim_end,'-');
xlabel('DA','FontWeight','demi','FontSize',12); 
ylabel('x','FontWeight','demi','FontSize',12);
hold on;line([D1_refer,D1_refer],[0,Mx_stim_end(OptiDA)],'line','--');hold off;
ylim([0,0.8]);xlim([-0.2,2.2]);
subplot(1,5,4);
errorbar(xx,Mux_stim_end,Sux_stim_end,'-');
xlabel('DA','FontWeight','demi','FontSize',12); 
ylabel('ux','FontWeight','demi','FontSize',12);
hold on;line([D1_refer,D1_refer],[0,Mux_stim_end(OptiDA)],'line','--');hold off;
ylim([0,0.4]);xlim([-0.2,2.2]);
subplot(1,5,5);
errorbar(xx,Muxf,Suxf,'-');
xlabel('DA','FontWeight','demi','FontSize',12); 
ylabel('uxf','FontWeight','demi','FontSize',12);
hold on;line([D1_refer,D1_refer],[0,Muxf(OptiDA)],'line','--');hold off;
