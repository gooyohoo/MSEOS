...
for JE2I=je2i
% for JE2I=85:5:115
  storageName = strcat('.\','huhu_win_critical_Jc_Pic2_D1_EE',num2str(JE2I),'_rest','\mingw5\branches_train.mat');
  load(storageName);
  num=numel(d1);
  order1=order0+num;
%   x(order0+1:order1)=JE2I;y(order0+1:order1)=d1;z(order0+1:order1)=fir_I;  
  x(order0+1:order1)=JE2I;y(order0+1:order1)=d1;z(order0+1:order1)=expect;  
...
xlabel('A_{E2I}');ylabel('A_{E2E}');zlabel('\sigma');
% title('3D fire rate transition of excitatoty');

figure();
set(gca,'DataAspectRatio',[2 1 2])
set(gcf,'color','w')  
[X,Y,Z]=griddata(x,y,z,linspace(min(x),max(x))',linspace(min(y),max(y)),'linear');%插值
pcolor(X,Y,Z);shading interp;colorbar; %伪彩色图
xlabel('A_{E2I} [%]','Fontname', 'Times Roman','FontSize',10);ylabel('A_{E2E} [%]','Fontname', 'Times Roman','FontSize',10);
set(gca, 'XTick',90:10:160,'Fontname', 'Helvetica','FontSize',10)
...
hold on;
plot(Ci_100,Ce_100,'-w','LineWidth',2.5);
